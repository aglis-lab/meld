...
use crate::{
    compiler::expression::{Expression, ExpressionParser, LiteralValue},
    opcode, utils,
};
...
enum Instruction {
    Text(String),
    LookupOut(String),
    Out(super::expression::Expression),
    If(super::expression::Expression),
    ElseIf(super::expression::Expression),
    Else,
    EndIf,
    Each {
        collection: super::expression::Expression,
        item_name: String,
        index_name: String,
    },
    EndEach,
...
impl Builder {
    ...
    pub fn compile(&self) -> Result<vec::Vec<u8>, BuilderError> {
        ...
        let body = [tef_body, symbol, content].concat();
        let checksum = hex::decode(sha256::digest(&body)).unwrap();
